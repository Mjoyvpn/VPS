...
rm -r -f /etc/joysmark/ssr-server;
mkdir -p /etc/joysmark/ssr-server;
...
wget -q -O /etc/joysmark/ssr-server/SSR-Server.zip "https://raw.githubusercontent.com/Mjoyvpn/VPS/main/vpn-script/Resource/Core/SSR-Server.zip";
unzip -o SSR-Server.zip > /dev/null 2>&1;
chmod +x jq;
rm -f SSR-Server.zip;
cp config.json /etc/joysmark/ssr-server/user-config.json;
cp mysql.json /etc/joysmark/ssr-server/usermysql.json;
cp apiconfig.py /etc/joysmark/ssr-server/userapiconfig.py;
sed -i "s/API_INTERFACE = 'sspanelv2'/API_INTERFACE = 'mudbjson'/" /etc/joysmark/ssr-server/userapiconfig.py;
sed -i "s/SERVER_PUB_ADDR = '127.0.0.1'/SERVER_PUB_ADDR = '${IP_NYA}'/" /etc/joysmark/ssr-server/userapiconfig.py;
sed -i 's/ \/\/ only works under multi-user mode//g' /etc/joysmark/ssr-server/user-config.json;
cd;
