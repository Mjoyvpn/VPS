...
mkdir -p /etc/joysmark/xray-mini-socks/;
...
cp /etc/xray-mini/socks.json /etc/joysmark/xray-mini-utils/socks-backup.json;
cat /etc/joysmark/xray-mini-utils/socks-backup.json | jq '.inbounds[0].settings.accounts += [{"user": "'${Username}'","pass": "'${Username}'" }]' > /etc/xray-mini/socks.json;
...
mkdir -p /etc/joysmark/socks/;
mkdir -p /etc/joysmark/socks/${Username};
rm -f /etc/joysmark/socks/${Username}/config.log;

# // Success
sleep 1;
clear;
echo -e "Your Premium Socks4/5 Proxy" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " IP          = ${IP_NYA}" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " Address     = ${domain}" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " Port        = ${port}" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " User        = ${Username}" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " Password    = ${Username}" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " SOCKS4/5 CONFIG LINK" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " ${tmp1}" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e "===============================" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " Created     = ${hariini}" | tee -a /etc/joysmark/socks/${Username}/config.log;
echo -e " Expired     = ${exp}";
echo -e "===============================";