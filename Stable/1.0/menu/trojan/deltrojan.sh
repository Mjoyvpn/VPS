...
cat /etc/joysmark/xray-mini-utils/tls-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.password == "'${client}'"))' > /etc/joysmark/xray-mini-cache.json;
cat /etc/joysmark/xray-mini-cache.json | jq 'del(.inbounds[1].settings.clients[] | select(.password == "'${client}'"))' > /etc/joysmark/xray-mini-cache2.json;
cat /etc/joysmark/xray-mini-cache2.json | jq 'del(.inbounds[4].settings.clients[] | select(.password == "'${client}'"))' > /etc/xray-mini/tls.json;
rm -rf /etc/joysmark/trojan/${client};
