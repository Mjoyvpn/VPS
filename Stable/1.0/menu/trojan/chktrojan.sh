...
echo -n > /etc/joysmark/cache/trojan_temp.txt;
echo -n > /etc/joysmark/cache/trojan_temp2.txt;
...
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/joysmark/cache/trojan_temp.txt;
else
echo "$ip" >> /etc/joysmark/cache/trojan_temp2.txt;
fi
jum2=$(cat /etc/joysmark/cache/trojan_temp.txt);
sed -i "/$jum2/d" /etc/joysmark/cache/trojan_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/joysmark/cache/trojan_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/joysmark/cache/trojan_temp.txt | nl);
hariini=`date -d "0 days" +"%Y-%m-%d"`;
echo "Username : $akun";
echo "$jum2";
echo ""
fi
rm -rf /etc/joysmark/cache/trojan_temp.txt;
done
echo "=============================================";
echo "            GRPC IP Login List";
echo "=============================================";
oth=$(cat /etc/joysmark/cache/trojan_temp2.txt | sort | uniq | nl );
echo "GRPC IP : All User IP";
echo "$oth";
echo "=============================================";
echo ""
rm -rf /etc/joysmark/cache/trojan_temp2.txt;