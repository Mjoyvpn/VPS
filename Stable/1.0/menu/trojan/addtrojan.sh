...
cat /etc/joysmark/xray-mini-cache.json | jq '.inbounds[1].settings.clients += [{"password": "'${Username}'","email":"'${Username}'" }]' > /etc/joysmark/xray-mini-cache2.json;
...
sleep 1;
clear;
echo -e "Your Premium Trojan Details" | tee -a /etc/joysmark/trojan/${Username}/config.log;
