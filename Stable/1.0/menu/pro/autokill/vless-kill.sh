...
# // Start
echo "$(date) Autokill Vless MultiLogin for joysmark Script V1.0 Stable";
...
sleep 30
# // Start
grep -c -E "^Vless " "/etc/xray-mini/client.conf" > /etc/joysmark/jumlah-akun-vless.db;
grep "^Vless " "/etc/xray-mini/client.conf" | awk '{print $2}'  > /etc/joysmark/akun-vless.db;
totalaccounts=`cat /etc/joysmark/akun-vless.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/joysmark/total-akun-vless.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/joysmark/akun-vless.db | tail -n 1 );
    ...
    if [[ $jumlah_baris_log -gt $VLESS ]]; then
        ...
        cat /etc/joysmark/xray-mini-utils/tls-backup.json | jq 'del(.inbounds[3].settings.clients[] | select(.email == "'${username}'"))' > /etc/joysmark/xray-mini-cache.json;
        cat /etc/joysmark/xray-mini-cache.json | jq 'del(.inbounds[6].settings.clients[] | select(.email == "'${username}'"))' > /etc/xray-mini/tls.json;
        cp /etc/xray-mini/nontls.json /etc/joysmark/xray-mini-utils/nontls-backup.json;
        cat /etc/joysmark/xray-mini-utils/nontls-backup.json | jq 'del(.inbounds[1].settings.clients[] | select(.email == "'${username}'"))' > /etc/xray-mini/nontls.json;
        rm -rf /etc/joysmark/vless/${username};
        ...
        echo "VLESS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/joysmark/autokill.log;
    fi
