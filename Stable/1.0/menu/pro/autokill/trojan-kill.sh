...
# // Start
echo "$(date) Autokill Trojan MultiLogin for joysmark Script V1.0 Stable";
...
do
    ...
    if [[ $jumlah_baris_log -gt $TROJAN ]]; then
        ...
        echo "TROJAN - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/joysmark/autokill.log;
    fi
# // End Function
done
done