#!/bin/bash
# (C) Copyright 2922-2023 By joysmark
# ==================================================================
...
# // Start
echo "$(date) Autokill Shadowsocks MultiLogin for joysmark Script V1.0 Stable";
...
sleep 30
# // Start
grep -c -E "^Shadowsocks " "/etc/xray-mini/client.conf" > /etc/joysmark/jumlah-akun-shadowsocks.db;
grep "^Shadowsocks " "/etc/xray-mini/client.conf" | awk '{print $2}'  > /etc/joysmark/akun-shadowsocks.db;
totalaccounts=`cat /etc/joysmark/akun-shadowsocks.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/joysmark/total-akun-trojan.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/joysmark/akun-shadowsocks.db | tail -n 1 );
    ...
    jumlah_baris_log=$( cat /etc/joysmark/xray-mini-shadowsocks/access.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );
    ...
    if [[ $jumlah_baris_log -gt $SHADOWSOCKS ]]; then
        ...
        cat /etc/joysmark/xray-mini-utils/shadowsocks-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${username}'"))' > /etc/xray-mini/shadowsocks.json;
        rm -rf /etc/joysmark/shadowsocks/${username};
        ...
        echo "SHADOWSOCKS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/joysmark/autokill.log;
    fi
# // End Function
done
done