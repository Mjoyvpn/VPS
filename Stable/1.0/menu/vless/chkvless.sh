...
echo -n > /etc/joysmark/cache/vless_temp.txt;
echo -n > /etc/joysmark/cache/vless_temp2.txt;
...
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/joysmark/cache/vless_temp.txt;
else
echo "$ip" >> /etc/joysmark/cache/vless_temp2.txt;
fi
jum2=$(cat /etc/joysmark/cache/vless_temp.txt);
sed -i "/$jum2/d" /etc/joysmark/cache/vless_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/joysmark/cache/vless_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/joysmark/cache/vless_temp.txt | nl);
...
echo "=============================================";
echo -n > /etc/joysmark/cache/vless_temp3.txt;
data=(`cat /etc/xray-mini/client.conf | grep '^Vless' | cut -d " " -f 2`);
for akun in "${data[@]}"
do
if [[ -z "$akun" ]]; then
...
echo "=============================================";
oth=$(cat /etc/joysmark/cache/vless_temp2.txt | sort | uniq | nl );
echo "GRPC IP : All User IP";
echo "$oth";
echo "=============================================";
echo "";
rm -rf /etc/joysmark/cache/vless_temp2.txt;
rm -rf /etc/joysmark/cache/vless_temp.txt;
rm -rf /etc/joysmark/cache/vless_temp3.txt;