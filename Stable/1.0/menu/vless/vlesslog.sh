...
# // Check Blacklist
export CHK_BLACKLIST=$( wget -qO- --inet4-only 'https://raw.githubusercontent.com/Mjoyvpn/VPS/main/vpn-script/blacklist.php?ip='"${IP_NYA}"'' );
...
if [[ -r /etc/joysmark/license-key.joysmark.me ]]; then
    SKIP=true;
else
    clear;
...
LCN_KEY=$( cat /etc/joysmark/license-key.joysmark.me | awk '{print $3}' | sed 's/ //g' );
if [[ $LCN_KEY == "" ]]; then
    clear;
...
export API_REQ_NYA=$( wget -qO- --inet4-only 'https://raw.githubusercontent.com/Mjoyvpn/VPS/main/vpn-script/secret/chk-rnn.php?scrty_key=61716199-7c73-4945-9918-c41133d4c94e&ip_addr='"${IP_NYA}"'&lcn_key='"${LCN_KEY}"'' );
...
cat /etc/joysmark/xray-mini-nontls/access.log | grep -w $hariini | grep -w $waktu | tail -n500 | grep -w 'accepted' | grep -w $client;
cat /etc/joysmark/xray-mini-tls/access.log | grep -w $hariini | grep -w $waktu | tail -n500 | grep -w 'accepted' | grep -w $client;
