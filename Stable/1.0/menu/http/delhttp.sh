...
export REPO_URL="https://raw.githubusercontent.com/Mjoyvpn/VPS/main/";
...
cat /etc/joysmark/xray-mini-utils/http-backup.json | jq 'del(.inbounds[0].settings.accounts[] | select(.user == "'${client}'"))' > /etc/xray-mini/http.json;
rm -rf /etc/joysmark/http/${client};
