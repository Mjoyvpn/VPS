...
do
    ...
    jumlah_baris_log=$( cat /etc/wildydev21/xray-mini-nontls/access.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );
    ...
    waktu=`date -d "0 days" +"%H:%M"`;
    waktunya=`date -d "0 days" +"%Y/%m/%d %H:%M"`;
    jumlah_baris_log=$( cat /etc/wildydev21/xray-mini-tls/access.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );
